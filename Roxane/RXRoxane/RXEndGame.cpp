        ...
        if(lower < upper) {
            ...
            for(RXSquareList* empties = board.empties_list->next; empties->position != NOMOVE; empties = empties->next)
                if(legal_movesBB & 0x1ULL<<empties->position) {
                    ...
                    move->score = (RXBitBoard::count_potential_moves(o_discs, p_discs)<<4)
                    - (RXBitBoard::get_corner_stability(p_discs)<<2)
                    - ((board.parity & RXBitBoard::QUADRANT_ID[move->position])>>RXBitBoard::QUADRANT_SHITF[move->position]);
                    
//                    move->score = (RXBitBoard::get_mobility(o_discs, p_discs)<<5)
//                    - (RXBitBoard::get_corner_stability(p_discs)<<2)
            ...
            if(list->next != NULL) { // nb moves > 0
                ...
                if((list->next)->next != NULL) { //nb moves > 1
                    ...
                    for(RXMove* iter = list->next; iter != NULL; iter = iter->next) {
                        ...
                        - (RXBitBoard::get_corner_stability(p_discs)<<1)
                        + RXBitBoard::count_potential_moves(o_discs, p_discs)
    ...
    if(bestmove != PASS) {
        ...
        for(RXSquareList* empties = board.empties_list->next; empties->position != NOMOVE; empties = empties->next)
            if(legal_movesBB & 1ULL<<empties->position){
                ...
                //synchronized acces
                if(!pv && hTable->get(board.hashcode_after_move(move), type_hashtable, entry) && entry.depth>=board.n_empties) {
                        
                    if ( entry.selectivity == NO_SELECT && -entry.upper >= upper )
                        return -entry.upper ;
        ...
        if(lower < upper && list->next != NULL) {
            ...
            if((list->next)->next != NULL) {
                ...
                for(RXMove* iter = list->next; iter != NULL; iter = iter->next) {
                    ...
                    iter->score += 2*(RXBitBoard::get_mobility(o_discs, p_discs)*VALUE_DISC)
                                - (RXBitBoard::get_corner_stability(p_discs)*VALUE_DISC/4)
...
int RXEngine::EG_PVS_deep(int threadID, RXBBPatterns& sBoard, const bool pv, const int selectivity, bool& selective_cutoff, int alpha, int beta, bool passed) {
    ...
    //    IID & IIS
    if(pv && bestmove == NOMOVE && board.n_empties > 16) {
        bool child_selective_cutoff = false;
        ...
        if(lower < upper && list->next != NULL) {
            ...
            if((list->next)->next != NULL) {
                ...
                if(board.n_empties > EG_DEEP_TO_MEDIUM) {
                    ...
                    probcut_bounds(board, std::max(0, (selectivity == NO_SELECT ? selectivity-2:selectivity-1)), ((board.n_empties & 0x1UL) == 0 ? 9:8), 0, (lower+upper)/2, lower_probcut, upper_probcut);
                    ...
                    for(RXMove* iter = list->next; iter!=NULL; iter = iter->next) {
                        ...
                        if(eval_move <= upper_probcut) {
                            eval_move /= 2;
                            eval_move -= 12*VALUE_DISC; //good move    "probable beta cut"    study in first
                        } else if(lower_probcut <= eval_move ){
                            eval_move /= 2;
                            eval_move += 12*VALUE_DISC; //bad move
                        }
            ...
            for(;!abort.load()  && lower < upper && list->next != NULL; list = list->next) {
                ...
                // Split?
                if(activeThreads > 1  && !abort.load() && (list->next)->next != NULL
                   && !thread_should_stop(threadID) && idle_thread_exists(threadID)
